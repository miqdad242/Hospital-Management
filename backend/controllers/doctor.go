...
func FilterRetrieve(w http.ResponseWriter, r *http.Request) {
	...
	err := db.QueryRow(ctx, "SELECT json_agg(t) as result from (SELECT * from doctor_filter_get($1,$2,$3)) t;", filter, page, rowCount).Scan(&outputStr)
	if err != nil {
...
func SaveDoctor(w http.ResponseWriter, r *http.Request) {
	...
	errs := validate.Var(output["doctor_name"], "required")
	if errs != nil {
	...
	_, err = db.Exec(ctx, "call doctor_save($1)", string(jsonb[:]))
	if err != nil {
		fmt.Println(err.Error())
		resp := u.Error("Failed SavingDoctor ")
		u.RespondJson(w, resp)
		return
	} else {
		id := "id"
		err := db.QueryRow(ctx, "Select last_gen_code From gen_codes Where gen_ref= 'MDR';").Scan(&id)
		if err != nil {
			fmt.Println(err.Error())
...
func DeleteDoctor(w http.ResponseWriter, r *http.Request) {
	...
	_, err = db.Exec(ctx, "call doctor_delete($1)", string(jsonb[:]))
	if err != nil {
...
func RetrieveDoctorList(w http.ResponseWriter, r *http.Request) {
	result := map[string]interface{}{}
	var outputStr string
	var output []interface{}
	err := db.QueryRow(ctx, "SELECT json_agg(t) as result from (SELECT * from doctor_get()) t;").Scan(&outputStr)
	if err != nil {
		fmt.Println(err.Error())
		resp := u.Error("Records not available")
		u.RespondJson(w, resp)
	...
	result["data"] = output
	resp := u.PayloadWithMessage(true, "Successfully Retrieved  doctors", result)
	u.RespondJson(w, resp)
}
...
func DoctorRetrievebyId(w http.ResponseWriter, r *http.Request) {
	result := map[string]interface{}{}
	var outputStr string
	var output []interface{}
	...
	err := db.QueryRow(ctx, "SELECT json_agg(t) as result from (SELECT * from doctor_get() where doctor_code =$1) t;", id).Scan(&outputStr)
	if err != nil {
		fmt.Println(err.Error())
		resp := u.Error("Records not available")
		u.RespondJson(w, resp)
