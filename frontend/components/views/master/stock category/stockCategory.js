...
const Index = () => {
    ...
    const [stockcategoryHeader, setstockcategoryHeader] = useState({})
    ...
    const onSubmit = async () => {
        ...
        if (formElement.checkValidity() === true) {
            let form = stockcategoryHeader
            if (form.stock_category_code == null || form.stock_category_code == '') {
                // POST (Create a New record) !!
                let { data, status, statusText } = await api.post(`/api/stock_category`, form)
                if (status === 200) {
    ...
    const fetchProfile = async (id) => {
        if (id === null) {
            ...
            id = id.trim()
            let { data, status, statusText } = await api.get(`/api/stock_categorybyid/${id}`)
            if (status === 200) {
                let response = (data.message);
                if (data.status === true) {
            ...
            <CCard className='m-4'>
                <CCardHeader className='pt-1 pb-1'>
                    <CRow className="align-items-center">
                        ...
                        <CCol md={7}>
                            <CFormLabel className="col-form-label fs-4">Stock Category</CFormLabel>
                        </CCol>
                        <CCol md={4} className='text-end pe-5'>
                            <CButton type="button" className=" col-form-label fs-6" color="success" variant='outline' onClick={() => { onSubmit(); }} >{id === null ? 'Add' : 'Update'}</CButton>
                        </CCol>
                    </CRow>
                </CCardHeader>
                    ...
                    <CForm ref={formHeader} id="FormHeader" className='needs-validation' noValidate validated={validated}>
                        <CRow className="mb-3">
                            <CCol md={1} className=' col-form-label'>
                                <CFormLabel >Stock Category Code</CFormLabel>
                            </CCol>
                            <CCol md={3}>
                                <CFormInput inline="true" readOnly
                                    type="text"
                                    name="stock_category_code"
                                    value={stockcategoryHeader.stock_category_code}
                                    onChange={({ target }) => { let p = { ...stockcategoryHeader }; p["stock_category_code"] = (target.value); setstockcategoryHeader(p); }}
                                />
                            </CCol>
                        </CRow>
                        <CRow className="mb-3">
                            ...
                            <CCol md={3}>
                                <CFormInput inline="true"
                                    ...
                                    value={stockcategoryHeader.stock_category_name}
                                    required
                                    onChange={({ target }) => { let p = { ...stockcategoryHeader }; p["stock_category_name"] = (target.value.trimStart()); setstockcategoryHeader(p); }}
