...
const Index = () => {
    ...
    const [Supplier_Groupe_Header, setHeader] = useState({})
    ...
    const onSubmit = async() =>{
        ...
        if (formElement.checkValidity() === true) {
            let form  =  Supplier_Groupe_Header
            if(form.supplier_group_code == null || form.supplier_group_code == '' ){
                // POST (Create a New record) !!
                let { data, status, statusText } = await api.post(`/api/supplier_groupe`,form)
                if (status === 200) {
      ...
      const fetchProfile = async(id) => {
        if(id === null){
            ...
            if (status === 200) {
                let response = (data.message);
                if (data.status === true) {
                    let suppliergroupObj = data.payload.data
                    loadField(suppliergroupObj);
                } else {
                    let toastFailMsg = (<ToastError response={response} />);
                    addToast(toastFailMsg);
                }
            ...
            <CCard className='m-4'>
                <CCardHeader className='pt-1 pb-1'>
                    <CRow className="align-items-center">
                        ...
                        <CCol md={7}> 
                            <CFormLabel className="col-form-label fs-4">Supplier Groupe</CFormLabel>
                        </CCol>
                ...
                <CCardBody style={{padding:'2rem'}}>
                    <CForm ref={formHeader} id="FormHeader" className='needs-validation' noValidate validated={validated}>
                        <CRow className="mb-3">
                            ...
                            <CFormLabel  >Supplier Groupe Code</CFormLabel>
                            </CCol>
                            <CCol md={3}> 
                                <CFormInput inline="true" readOnly 
                                    type="text"
                                    name="supplier_group_code"
                                    value={Supplier_Groupe_Header.supplier_group_code}
                                    onChange={({ target }) => { let p = { ...Supplier_Groupe_Header }; p["supplier_group_code"] = (target.value); setHeader(p); }}
                                />
                            </CCol>
                            </CRow>
                            <CRow className="mb-3">
                            <CCol md={2} className=' col-form-label'>
                            <CFormLabel style={{ width: '220px' }} >Supplier Groupe name</CFormLabel>
                            </CCol>
                            <CCol md={3}>
                                <CFormInput inline="true"
                                    type="text"
                                    name="supplier_group_name"
                                    value={Supplier_Groupe_Header.supplier_group_name}
                                    required
                                    onChange={({ target }) => { let p = { ...Supplier_Groupe_Header}; p["supplier_group_name"] = (target.value.trimStart()); setHeader(p); }}
