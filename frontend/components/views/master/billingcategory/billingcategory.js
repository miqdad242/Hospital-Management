...
const Index = () => {
  ...
  const onSubmit = async () => {
    ...
    if (formElement.checkValidity() === true) {
      let form = billHeader;
      if (form.bill_code == null || form.bill_code == "") {
        // POST (Create a New record) !!
        let { data, status, statusText } = await api.post(`/api/bill`, form);
        if (status === 200) {
  ...
  const locationDirect = () => {
    navigate("/master/billcategory");
  };
  const loadField = (obj) => {
    let data = {};
    for (let i = 0; i < obj.length; i++) {
      data.bill_code = obj[i].bill_code;
      data.bill_name = obj[i].bill_name;
    }
  ...
  const fetchProfile = async (id) => {
    if (id === null) {
      ...
      id = id.trim();
      let { data, status, statusText } = await api.get(`/api/bill/${id}`);
      if (status === 200) {
        let response = data.message;
        if (data.status === true) {
          let billObj = data.payload.data;
          loadField(billObj);
        } else {
          let toastFailMsg = <ToastError response={response} />;
          addToast(toastFailMsg);
      ...
      <CCard className="m-4">
        <CCardHeader className="pt-1 pb-1">
          <CRow className="align-items-center">
            ...
            <CCol md={7}>
              <CFormLabel className="col-form-label fs-4">Bill</CFormLabel>
            </CCol>
          ...
          >
            <CRow className="mb-3">
              <CCol md={1} className=" col-form-label">
                <CFormLabel>Bill Code</CFormLabel>
              </CCol>
              <CCol md={3}>
                <CFormInput
                  inline="true"
                  ...
                  type="text"
                  name="bill_code"
                  value={billHeader.bill_code}
                  onChange={({ target }) => {
                    let p = { ...billHeader };
                    p["bill_code"] = target.value;
                    setBillHeader(p);
                  }}
                />
              </CCol>
            ...
            <CRow className="mb-3">
              ...
              <CCol md={3}>
                <CFormInput
                  ...
                  type="text"
                  name="Bill_name"
                  value={billHeader.bill_name}
                  required
                  onChange={({ target }) => {
                    let p = { ...billHeader };
                    p["bill_name"] = target.value.trimStart();
                    setBillHeader(p);
                  }}
                  feedbackInvalid="Bill Name Is Required."
                />
              </CCol>
            </CRow>
          </CForm>
